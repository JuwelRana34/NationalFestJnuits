...
// const DynamicDataRenderer = ({ data }: { data: any }) => {
...
//     return (
//       <span className="text-sm font-medium text-gray-900 break-word">
//         {String(data)}
//       </span>
//     );
//   }
...
//   const [selectedReg, setSelectedReg] = useState<MappedRegistration | null>(null);

//   useEffect(() => {
//     const fetchRegistrations = async () => {
//       try {
//         const { status, response } = await honoFetch<ApiResponse>("/api/registrations");
...
//                 category: meta?.eventType || "General",
...
//         }
//       } catch (error) { // 💡 FIXED: Removed ": any" from catch block
...
//       (reg.transactionId && reg.transactionId.toLowerCase().includes(searchQuery.toLowerCase()));
...
//         prev.map((reg) =>
//           reg.id === id ? { ...reg, selectionStatus: newStatus } : reg,
//         ),
//       );
//       try {
//         console.log(`Status updated to ${newStatus} for Registration ID: ${id}`);
//         const { status, response } = await honoFetch<ApiResponse>(`/api/registrations/${id}/status`, {
//           method: "PATCH",
...
//                             <p
//                               className={`font-semibold ${payment.status === "VERIFIED" ? "text-green-600" : payment.status === "REJECTED" ? "text-red-600" : "text-yellow-600"}`}
//                             >
//                               {payment.status}
//                             </p>
//                           </div>
