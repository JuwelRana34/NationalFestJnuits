...
export default function App() {
  ...
  return (
    <section className="min-h-screen w-full bg-background text-foreground py-24 px-6 md:px-12 font-sans relative overflow-hidden font-inter">
      {/* Background glows */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            viewport={{ once: true }}
            className="hidden md:flex items-center gap-2 px-6 py-3 rounded-md font-medium transition-all border border-border hover:bg-muted text-foreground"
          >
          ...
          {segments.map((segment) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative bg-card border border-border p-6 rounded-md flex flex-col h-full overflow-hidden cursor-pointer transition-all duration-500 hover:-translate-y-2 shadow-lg ${styles.shadow}`}
              >
                {/* Hover gradient */}
                <div
                ...
                <div className="relative z-10 flex flex-col h-full">
                  <div className="mb-6">
                    <div
                      className={`w-14 h-14 rounded-md flex items-center justify-center bg-muted border border-border group-hover:border-transparent transition-all duration-500 ${styles.iconBg}`}
                    >
                      <Icon
                  ...
                  <p className="text-muted-foreground text-sm leading-relaxed grow mb-8 transition-colors duration-300">
                    {segment.description}
                  </p>

                  <div
                    className={`mt-auto flex items-center gap-2 font-medium text-sm ${styles.text}`}
                  >
        ...
        <motion.button
          ...
          viewport={{ once: true }}
          className="mt-10 w-full md:hidden flex justify-center items-center gap-2 px-6 py-4 rounded-md font-medium border border-border hover:bg-muted transition-colors"
        >
          View Schedule <ArrowRight className="w-4 h-4" />
        </motion.button>
      </div>
