...
export default function HeroSection() {
  return (
    // 1. MAIN WRAPPER
    // 'relative' keeps absolute background elements contained here.
    // 'overflow-hidden' prevents the blurry background glow from causing horizontal scrollbars.
    <main className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-zinc-950 selection:bg-cyan-500/30">
      {/* 2. BACKGROUND EFFECTS */}
      {/* Grid Pattern: Uses pure Tailwind linear-gradients to draw lines instead of a custom CSS class */}
      <div className="pointer-events-none absolute inset-0 z-0 bg-[linear-gradient(to_right,#80808020_1px,transparent_1px),linear-gradient(to_bottom,#80808020_1px,transparent_1px)] bg-[size:40px_40px]" />

      {/* Radial Glow: Creates a soft cyan spotlight behind the text */}
      <div className="pointer-events-none absolute left-1/2 top-[-10%] z-0 h-[400px] w-[300px] -translate-x-1/2 rounded-full bg-cyan-500/20 blur-[120px] sm:h-[600px] sm:w-[600px]" />

      {/* 3. MAIN CONTENT CONTAINER */}
      {/* 'z-10' ensures the text stays on top of the background effects */}
      <section className="relative -mb-16 z-10 mx-auto flex w-full max-w-4xl flex-col items-center px-6 text-center">
        {/* Eyebrow Badge */}
        <div className="mb-8 inline-flex cursor-default items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-4 py-2 text-sm font-medium text-cyan-300 backdrop-blur-sm transition-colors hover:bg-cyan-500/20">
          <Sparkles className="h-4 w-4" />
          <span>National AI & IT Summit 2026</span>
        </div>

        {/* Main Headline */}
        {/* Added a text gradient to 'AI & IT builders' to make it pop visually */}
        <h1 className="text-4xl font-extrabold tracking-tight text-white drop-shadow-md sm:text-6xl md:text-7xl">
          The flagship summit for <br className="hidden sm:block" />
        ...
        {/* Description */}
        <p className="mt-6 max-w-2xl text-base leading-relaxed text-zinc-400 sm:text-lg md:text-xl">
          Three days of cutting-edge competitions, visionary keynotes, and
          interactive showcases. Explore the full programme and claim your spot
          among industry leaders.
        </p>

        {/* Event Metadata (Date & Location) */}
        {/* Grouping these in a frosted-glass container looks highly professional */}
        <div className="mt-8 flex flex-col items-center gap-4 rounded-2xl border border-zinc-800 bg-zinc-900/50 px-6 py-3 text-sm font-medium text-zinc-300 backdrop-blur-sm sm:flex-row sm:gap-8">
          <div className="flex items-center gap-2">
            <Calendar className="h-4 w-4 text-cyan-400" />
            <span>March 14–16, 2026</span>
          </div>
          {/* Decorative dot separator (hidden on mobile) */}
        ...
        <div className="mt-10 flex w-full flex-col items-center gap-4 sm:w-auto sm:flex-row">
          ...
          asChild
            size="lg"
            className="h-12 w-full bg-cyan-500 font-semibold text-zinc-950 transition-all hover:bg-cyan-400 hover:shadow-[0_0_24px_rgba(34,211,238,0.4)] sm:w-auto"
          >
            <Link href="/events">
              Explore events
          ...
          {/* Secondary Button */}
          {/* A ghost/outline button provides a clear secondary action without competing visually */}
          <Button
          asChild
            size="lg"
            variant="outline"
            className="h-12 w-full border-zinc-700 bg-transparent text-white transition-all hover:bg-zinc-800 hover:text-white sm:w-auto"
          >
